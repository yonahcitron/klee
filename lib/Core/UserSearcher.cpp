...
cl::list<Searcher::CoreSearchType> CoreSearch(
    ...
    cl::values(
        ...
        clEnumValN(Searcher::ParserGuided, "parser-guided",
                   "use three-tier parser-guided search (known-value DFS + "
                   "BFS + covnew)")),
    cl::cat(SearchCat));
...
bool userSearcherRequiresMD2U() {
  return (std::find(CoreSearch.begin(), CoreSearch.end(), Searcher::NURS_MD2U) != CoreSearch.end() ||
          ...
          std::find(CoreSearch.begin(), CoreSearch.end(), Searcher::ParserGuided) != CoreSearch.end());
}
...
Searcher *getNewSearcher(Searcher::CoreSearchType type, RNG &rng,
  ...
  switch (type) {
    ...
    case Searcher::ParserGuided: searcher = new ParserGuidedSearcher(rng); break;
  }
