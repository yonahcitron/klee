...
Eq-pinned generation steers by the values the program *compared the frontier
byte against* — pFuzzer's directed signal — instead of by concrete ktest bytes
(take-all) or a wrapped strcmp proxy. KLEE already has that signal: every time
the parser tests an input byte it leaves an ``Eq(C, Read(i, win))`` in the path
condition. This module pulls those pins out of the ``.kquery`` dumps KLEE writes
with ``--write-kqueries``.
...
(the ``whila`` / ``whil@`` junk pFuzzer never generates). Distinguishing the two
at the source is what makes directed generation directed.
...
tiny.c / luac lexers produce). Multi-byte word compares that reach the solver as
Concat(Read,Read,...) (yyjson byte_match_4) are out of scope — that lexer class
is not an eq-pinned subject.
"""
